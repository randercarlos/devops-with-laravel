...
ssh -tt -o StrictHostKeyChecking=no -i $HOME/.ssh/id_rsa $SSH_USER@$SSH_SERVER "chmod +x ./restore.sh && ./restore.sh $S3_FULL_URI_BACKUP_FILENAME $MYSQL_USER $MYSQL_PASSWORD"
